 ...
 * for the same student/day. Per-student aggregations (StudentProfilePage,
 * Reports per-student rows) would double-count that day.
 *
 * Apply at READ sites where the same student's attendance is being
 * aggregated across classes. DO NOT apply at class-scoped read sites
 * (those are inherently single-class so doc-id prevents dups).
 *
 * Mirrors parent-dashboard + teacher-dashboard helpers — same conflict
 * rule (latest createdAt wins) so all dashboards stay consistent.
 */
