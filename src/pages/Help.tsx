...
export default function Help() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      await createTicket({
        schoolId,
        branchId: String(userData?.branchId || userData?.branch || ""),
        branchName: String(userData?.branchName || userData?.branch || ""),
        schoolName: String(userData?.schoolName || ""),
        createdBy: {
          uid: user.uid,
          email: (user.email || "").toLowerCase(),
          name: userData?.name || user.displayName || "Principal",
          role: "principal",
        },
        subject,
        description,
            ...
            <ul className="space-y-2">
              {filtered.map((t) => (
                <TicketRow
                  ...
                  currentUid={user?.uid || ""}
                  currentName={
                    userData?.name || user?.displayName || "Principal"
                  }
