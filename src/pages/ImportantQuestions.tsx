...
const ImportantQuestions = () => {
  ...
  // classIds is an array — principal can attach one upload to MULTIPLE classes
  // in a single action. We still write one Firestore doc per class (each with
  // its own `classId`) so readers on the teacher/parent side don't need to
  // change. The file is uploaded once and shared across all docs via fileUrl.
  const [uploadForm,    setUploadForm]    = useState<{
    ...
    academicYear: string;
    file: File | null;
  }>({ classIds: [], subject: "", title: "", academicYear: "", file: null });
  ...
  const handleDelete = async (s: ImportantQuestionDoc) => {
    ...
    setDeletingId(s.id);
    try {
      // Multi-class uploads share one file across N docs. Only delete the
      // Storage object when this is the LAST doc pointing to it — otherwise
      // sibling class views would 404.
      if (s.filePath) {
        const siblingCount = syllabi.filter(o => o.filePath === s.filePath && o.id !== s.id).length;
        if (siblingCount === 0) {
          try {
  ...
  const resetUploadForm = () => {
    setUploadForm({ classIds: [], subject: "", title: "", academicYear: "", file: null });
    setUploadProgress(0);
  ...
  const handleUpload = async () => {
    ...
    const { classIds, subject, title, academicYear, file } = uploadForm;
    if (!classIds || classIds.length === 0) { toast.error("Please select at least one class."); return; }
    ...
    const targetClasses = classes.filter(c => classIds.includes(c.id));
    if (targetClasses.length === 0) { toast.error("Selected classes no longer exist."); return; }
    ...
    try {
      ...
      // Write one Firestore doc per class so existing readers
      // (`where("classId","==",X)`) work unchanged. `classIds` array is
      ...
      >
        ...
        <div style={{ padding: 22, display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          <div>
            <label style={{ fontSize: 11, fontWeight: 700, color: "#002080", letterSpacing: "0.04em", textTransform: "uppercase", display: "block", marginBottom: 6 }}>
              Classes <span style={{ color: "#FF3355" }}>*</span>
              <span style={{ marginLeft: 8, fontWeight: 600, color: "#5070B0", textTransform: "none", letterSpacing: 0 }}>
                (select one or more)
              </span>
            </label>
            <button
              ...
              style={{
                ...
                fontSize: 14,
                fontWeight: 600,
                cursor: uploading || classes.length === 0 ? "not-allowed" : "pointer",
                textAlign: "left",
                display: "flex",
                alignItems: "center",
                flexWrap: "wrap",
            ...
            >
              {uploadForm.classIds.length === 0 ? (
                <span style={{ color: "#99AACC", fontWeight: 500 }}>— Select classes —</span>
              ) : (
                <>
                  {uploadForm.classIds.slice(0, 4).map(id => {
                      ...
                      >
                        ...
                        <span
                          onClick={(e) => {
                            e.stopPropagation();
                            if (uploading) return;
                            setUploadForm(f => ({ ...f, classIds: f.classIds.filter(x => x !== id) }));
                          }}
                          style={{ cursor: "pointer", marginLeft: 2, opacity: 0.8, fontWeight: 700 }}
                  ...
                  {uploadForm.classIds.length > 4 && (
                    <span style={{ padding: "3px 10px", background: "rgba(0,85,255,0.10)", color: "#0055FF", borderRadius: 100, fontSize: 12, fontWeight: 700 }}>
                      +{uploadForm.classIds.length - 4} more
                    </span>
                  )}
                </>
              )}
              <span style={{ marginLeft: "auto", color: "#5070B0", fontSize: 12 }}>
                {classPickerOpen ? "▴" : "▾"}
              </span>
            </button>
            {classPickerOpen && classes.length > 0 && (
              ...
              >
                <div style={{ display: "flex", gap: 6, padding: "8px 10px", borderBottom: "0.5px solid rgba(0,85,255,0.08)" }}>
                  <button
                    type="button"
                    onClick={() => setUploadForm(f => ({ ...f, classIds: classes.map(c => c.id) }))}
                    disabled={uploading}
                    style={{ flex: 1, padding: "6px 10px", fontSize: 11, fontWeight: 700, color: "#0055FF", background: "rgba(0,85,255,0.06)", border: "0.5px solid rgba(0,85,255,0.16)", borderRadius: 8, cursor: "pointer", fontFamily: "inherit" }}
                  >
                    Select all ({classes.length})
                  </button>
                  <button
                    type="button"
                    onClick={() => setUploadForm(f => ({ ...f, classIds: [] }))}
                    disabled={uploading || uploadForm.classIds.length === 0}
                    style={{ flex: 1, padding: "6px 10px", fontSize: 11, fontWeight: 700, color: "#FF3355", background: "rgba(255,51,85,0.06)", border: "0.5px solid rgba(255,51,85,0.16)", borderRadius: 8, cursor: "pointer", opacity: uploadForm.classIds.length === 0 ? 0.5 : 1, fontFamily: "inherit" }}
                  >
                    Clear
                  </button>
                </div>
                {classes.map(c => {
                  const checked = uploadForm.classIds.includes(c.id);
                  return (
                    <label
                      key={c.id}
                      style={{
                        ...
                        padding: "10px 12px",
                        cursor: "pointer",
                        background: checked ? "rgba(0,85,255,0.06)" : "transparent",
                        borderBottom: "0.5px solid rgba(0,85,255,0.04)",
                      }}
                    >
                      <input
                        type="checkbox"
                        checked={checked}
                        onChange={() => setUploadForm(f => ({
                          ...f,
                          classIds: checked
                            ? f.classIds.filter(x => x !== c.id)
                            : [...f.classIds, c.id],
                        }))}
                        disabled={uploading}
                        style={{ accentColor: "#0055FF", width: 16, height: 16 }}
                      />
                      <span style={{ fontSize: 13, fontWeight: 600, color: "#001040" }}>{c.name}</span>
                    </label>
                  );
                })}
              </div>
        ...
        >
          ...
          <button
            ...
            disabled={uploading || uploadForm.classIds.length === 0 || !uploadForm.title.trim() || !uploadForm.file}
            style={{
              flex: 1.4,
              height: 44,
              borderRadius: 12,
              ...
              justifyContent: "center",
              gap: 8,
              opacity: (uploading || uploadForm.classIds.length === 0 || !uploadForm.title.trim() || !uploadForm.file) ? 0.6 : 1,
              fontFamily: "inherit",
            }}
          >
            {uploading
